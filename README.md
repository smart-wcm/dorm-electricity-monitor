...
- 从本仓库 **Releases 页面**下载 `dorm-elec-monitor_v2.0.1_onefile.exe`（单文件）或 `dorm-elec-monitor_v2.0.1_onedir.zip`（目录版），双击即用，**无需安装 Python / 任何依赖**。
...
| | `dorm-elec-monitor_v2.0.1_onefile.exe`（单文件版） | `dorm-elec-monitor_v2.0.1_onedir.zip`（目录版） |
|---|---|---|
...
![托盘图标：绿色闪电 = 最近一次抓取成功](assets/tray-icon.png)

![托盘右键菜单：立即抓取 / 打开网页报告 / 开机自启 / 查看运行日志 / 退出监控](assets/tray-menu.png)
...
> GitHub Releases 上发布的两个资产即上述两种产物：`dorm-elec-monitor_v2.0.1_onefile.exe`（单文件版）与 `dorm-elec-monitor_v2.0.1_onedir.zip`（目录版压缩包），对应关系见「六、安装、配置与运行 → 方式 A」。
